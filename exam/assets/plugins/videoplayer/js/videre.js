	...
	var Video = {
		...
		wrapPlayer: function (el) {
			...
			template = $(
				...
				'<video src="' + base.options.video.quality[0].src + '" id="html-player" style="width: 100%; height: ' + dimensions[base.options.dimensions][1] + 'px;"></video>' +
				'</div>' +
				'<div class="loading-holder" id="loading-video-hldr"><div class="spinner-border text-primary" style="width: 4rem; height: 4rem; color: #fefefe !important;" role="status"><span class="sr-only">Loading...</span></div></div>' +
				'<div class="family-to-family-logo"><figure><img class="f2f-logo-main-v2" src="' + base_url + 'assets/img/f2f-main-logo-v2.png"></figure></div>' +
		...
		decodeMedia: function (el) {
			...
			media.onwaiting = function () {
				...
				// $('.vid-toggle-layer').append('<div class=""></>');\
				$('#loading-video-hldr').css('display','block');
			};
			media.onended = function () {
				// while video has ended
				$('.family-to-family-logo').fadeIn();
			...
			media.oncanplaythrough = function () {
				...
				$('#loading-video-hldr').css('display','none');
		...
		renderProgress: function () {
			// viewed_video_check = `<i class="fas fa-video-slash"></i>`;
			var base = this;
			...
			// modified
			// var countprogress = (100 / media.duration) * media.currentTime;
			// if (countprogress == 100) {
				
			// } else {
				
			// 	// console.log(sessionStorage.getItem("file_icon"));
			// 	// do nothing
			// }
			if (media.duration)
